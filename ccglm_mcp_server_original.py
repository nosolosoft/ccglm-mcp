...
# Configuración de timeouts sincronizada con Claude Code (300s)
DEFAULT_TIMEOUT = 280    # 4.6 minutos (un poco menos que Claude para evitar race conditions)
MAX_TIMEOUT = 295        # 4.9 minutos (margen de seguridad antes del timeout de Claude)
CONNECTION_TIMEOUT = 15  # 15 segundos para startup de subprocess

# Timeouts específicos por modelo para optimización (siempre < 295s)
MODEL_TIMEOUTS = {
    "glm-4.5-air": 120,   # 2 minutos para modelo rápido
    "glm-4.6": 280       # 4.6 minutos para modelo completo (alineado con Claude)
}
...
# Process Manager para evitar leaks y manejar cleanup
class ProcessManager:
    """Gestor centralizado de procesos con cleanup garantizado"""
    ...
    async def create_process(self, *args, **kwargs) -> asyncio.subprocess.Process:
        """Crear proceso y registrar para cleanup automático"""
        process = await asyncio.create_subprocess_exec(*args, **kwargs)
        self.active_processes.add(process)
        logger.debug(f"📝 Process {process.pid} registered with ProcessManager")
        return process

    async def cleanup_process(self, process: asyncio.subprocess.Process):
        """Cleanup individual con manejo robusto de errores"""
        ...
        try:
            ...
            if process.returncode is None:  # Still running
                ...
                try:
                    ...
                    process.kill()
                    await process.wait()
                    logger.debug(f"💀 Process {process.pid} force killed")
            else:
    ...
    async def shutdown_all(self):
        """Shutdown de todos los procesos con cleanup garantizado"""
        async with self.shutdown_lock:
            if self._shutting_down:
                return
            self._shutting_down = True

            logger.info(f"🛑 ProcessManager shutting down {len(self.active_processes)} processes...")

            # Crear copia para evitar modificar set durante iteración
            processes_to_cleanup = list(self.active_processes)

            # Cleanup concurrente con timeout
            cleanup_tasks = [self.cleanup_process(p) for p in processes_to_cleanup]
            if cleanup_tasks:
                try:
                    ...
                    logger.warning("⏰ Process shutdown timeout, some processes may remain")

            self.active_processes.clear()
            logger.info("✅ ProcessManager shutdown complete")

# Instancia global del ProcessManager
process_manager = ProcessManager()

# PID file locking para evitar instancias múltiples
PID_FILE = "/tmp/ccglm-mcp.pid"
_pid_file_handle = None

def acquire_pid_lock() -> bool:
    """Adquirir lock exclusivo usando PID file"""
    ...
    try:
        ...
        logger.info(f"🔒 PID lock acquired for process {os.getpid()}")
        return True
    except (IOError, OSError):
        if _pid_file_handle:
            _pid_file_handle.close()
            _pid_file_handle = None
        logger.warning("⚠️ Another CCGLM MCP instance is already running")
        return False

def release_pid_lock():
    """Liberar PID file lock"""
    global _pid_file_handle
    try:
        if _pid_file_handle:
            fcntl.flock(_pid_file_handle.fileno(), fcntl.LOCK_UN)
            _pid_file_handle.close()
            _pid_file_handle = None
...
def setup_signal_handlers():
    ...
    def signal_handler(signum, frame):
        ...
        asyncio.create_task(process_manager.shutdown_all())
        release_pid_lock()
        sys.exit(0)
...
# Registration de cleanup al exit
atexit.register(lambda: asyncio.run(process_manager.shutdown_all()))
...
async def ccglm_route(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Crear proceso con Process Manager y comunicación stdin
        process = None
        try:
            ...
            logger.info(f"🎯 Process {process.pid} created via ProcessManager")
        except asyncio.TimeoutError:
        ...
        try:
            ...
            logger.warning(f"⏰ GLM process timeout after {effective_timeout}s for model {model}")
            return {"error": f"Request timed out after {effective_timeout}s for model {model}"}
        except Exception as e:
            logger.error(f"❌ Unexpected error in GLM communication: {e}")
            return {"error": f"Communication error: {str(e)}"}
        finally:
            # Cleanup garantizado del proceso via ProcessManager
            if process:
                await process_manager.cleanup_process(process)
...
async def main():
    """Main entry point"""
    # Adquirir PID lock para evitar instancias múltiples
    if not acquire_pid_lock():
        logger.error("❌ Another CCGLM MCP instance is already running")
        sys.exit(1)
